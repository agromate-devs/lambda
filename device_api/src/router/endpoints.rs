...
fn query_to_string(items: &[HashMap<String, AttributeValue>]) -> String {
    ...
    for item_raw in items {
        for element in item_raw {
            let item = Item {
                key: element.0.to_string(),
                value: element.1.as_s().unwrap().to_string(),
            };
            items_parsed.push(item);
        }
    }
    let items_dedup = items_parsed.dedup_by(|a, b| a.value == b.value);
    serde_json::to_string(&items_dedup).unwrap()
}
